        ...
        @foreach($announcements as $announcement)
            ...
            @if($isCreator)
                <form class="relative" action="{{route('announcement.destroy', [$announcement->id]) }}" method="POST" onsubmit="return confirm('Are you sure you wish to delete this announcement?');">
                    ...
                    hover:shadow-lg m-2 h-10 flex justify-center items-center font-bold hover:bg-gradient-to-tr from-slate-800 to-slate-950 transition duration-500 hover:text-[#FEB71C] hover:border-[#FEB71C]"> DELETE </button>
                </form>
            @endif
        @endforeach
    </div>
